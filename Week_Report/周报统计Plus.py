...
def xianjindai_week():
    ...
    # print(df_all.sort_values(by='姓名'))
    # exit()

    # 根据2列汇总
    df_all = df_all.groupby(['阶段', '姓名']).sum()
    ...
    for x, y in df.groupby(['阶段']):
        ...
        df.loc[ii, '姓名'] = y.loc[0, '阶段']
        # df.loc[ii, ''] = y['姓名'].count()
    ...
    if week == str(5):
        ...
        df.to_excel('C:\\Users\Administrator\Desktop\{}贷催收{}月第{}周-周报.xlsx'.format(bm, yue, week), index=False)
